 ...
 * 		pythonw go.py <argv>
 * on Windows. "go.py" must be in the same directory.
 *
 * Rationale:
 ...
 *    - Want a subsystem:window mechanism with which to launch go.py for
 *      DQSD integration.
 */
...
int main(int argc, char** argv)
{
    ...
    snprintf(pyFile, MAXPATHLEN, "%s%cgo.py", programPath, SEP);
    
    // Build the argument array for launching.
    char* pythonArgs[MAX_PYTHON_ARGS+1];
    int nPythonArgs = 0;
    pythonArgs[nPythonArgs++] = "pythonw.exe";
