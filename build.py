...
"""
    ...
    This is the primary build script for the 'go' project. It exists
...
if sys.platform == "win32" and sys.version.startswith("2.3."):
    import warnings
    warnings.filterwarnings("ignore", module="termios", lineno=7)
import getpass
import shutil
import glob
import logging
...
def target_all():
    ...
    target_sdist()
    if sys.platform == "win32":
        log.warn("skipping 'webdist' target on win32")
    else:
        target_webdist()
...
def target_go_script():
    ...
    assert sys.platform != "win32", "'go_script' no makie sensie on Windows"
...
def target_upload_local():
    ...
    This is different from the "upload" target, which uploads release bits
    remotely to trentm.com.
    """
